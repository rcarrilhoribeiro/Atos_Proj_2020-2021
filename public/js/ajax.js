...
function showEmails(str, dataList, inputfield){
  xmlHttp = new XMLHttpRequest();
  var url = "/ajax/user-emails";
  url += "?email=" + str;
  xmlHttp.onreadystatechange = function () {
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
      jsonToEmail(xmlHttp.responseText, dataList);
      inputfield.placeholder = "insert keyword...";
      inputfield.focus();
    } else inputfield.placeholder = "Couldn't load datalist options :(";
  };
...
function jsonToEmail(responseText, dataList) {
  dataList.innerHTML = "";
  var jsonOptions = JSON.parse(responseText);
  // console.log(jsonOptions);
  // Loop over the JSON array.
